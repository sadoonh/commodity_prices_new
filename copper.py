...
def load_and_preprocess_data(ticker="HG=F"):
    copper = yf.Ticker(ticker)
    hist = copper.history(period="max")
    df = hist[['Close']].copy()
...
def train_or_load_model(X_train, y_train, model_path="lstm_model.h5", mode='load'):
...
def save_forecast_to_csv(df, forecast, output_file="copper_forecast.csv"):
...
def plot_results(df, forecast, scaler):
    ...
    plt.title('Copper Prices: Historical Data (Last 12 Months), Moving Average, and 90-Day Forecast')
    plt.xlabel('Date')
...
if __name__ == "__main__":
    ...
    model = train_or_load_model(X_train, y_train, mode='load')

    # Forecast next 90 days
    forecast = forecast_next_days(model, data, scaler, time_step, days=90)

    # Save forecast data to CSV
    save_forecast_to_csv(df, forecast, output_file="copper_forecast.csv")
